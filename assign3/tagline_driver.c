...
//		}
	...
	RAIDOpCode resp = raid_bus_request(request, buf[c]);
	int result = extract_raid_response(resp);
	if (result == 1){ //operation failed
		return (-1);
		}
...
int tagline_write(TagLineNumber tag, TagLineBlockNumber bnum, uint8_t blks, char *buf) {
	...
	for (b=0; b<a; b++){
		...
		RAIDOpCode resp = raid_bus_request(request, buf[b]);
		int result = extract_raid_response(resp);
		if (result == 1){ //operation failed
			return (-1);}
	}
	...
	for (b<blks; b++; bnum+=b){
		...
		RAIDOpCode resp2 = raid_bus_request(request2, buf[b]);
		int result2 = extract_raid_response(resp2);
		if (result2 == 1){ //operation failed
			return (-1);
		}
		...
		RAIDOpCode resp3 = raid_bus_request(request3, buf[b]);
...
int raid_disk_signal(void){
	...
	for (i=0; i<RAID_DISKS; i++){
		RAIDOpCode request = create_raid_request(RAID_STATUS, 0, 0, i, 0);		     RAIDOpCode resp = raid_bus_request(request, NULL);
		int result = extract_raid_response(resp);
	...
	//first, reformat everything in that disk to 0
		...
		for (j=0; j<RAID_DISKBLOCKS; j++){
			...
			char b;
			RAIDOpCode resp3= raid_bus_request(request3, b);
			...
			RAIDOpCode resp4 = raid_bus_request(request4, b);
